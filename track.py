...
from yolov7.utils.datasets import VID_FORMATS, LoadImages, LoadStreams
from yolov7.utils.general import (LOGGER, check_img_size, non_max_suppression, scale_coords, check_requirements, cv2,
                                  check_imshow, xyxy2xywh, increment_path, strip_optimizer, colorstr, print_args,
                                  check_file)
from yolov7.utils.torch_utils import select_device, time_sync, TracedModel
...
def run(
        source='0',
        yolo_weights=WEIGHTS / 'yolov7m.pt',  # model.pt path(s),
    ...
    model = attempt_load(yolo_weights, device)
    # stride, names = model.stride, model.names

    # 开启了过后运行速度反而降低了
    # if trace:
    #     model = TracedModel(model, device, imgsz[0])
    ...
    for frame_idx, (path, im, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            curr_frames[i] = im0
            if(txt_file_name=='1'):
                txt_file_name='MOT16-02'
            elif(txt_file_name=='2'):
                txt_file_name = 'MOT16-04'
            elif(txt_file_name=='3'):
                txt_file_name = 'MOT16-05'
            elif(txt_file_name=='4'):
                txt_file_name = 'MOT16-09'
            elif(txt_file_name=='5'):
                txt_file_name = 'MOT16-10'
            ...
            print('%sDone. ' % ( s ), f'YOLO:({t3 - t2:.3f}s), StrongSORT:({t5 - t4:.3f}s)')
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=str, default=ROOT / '', help='model.pt path(s)')
    parser.add_argument('--strong-sort-weights', type=str, default=ROOT / '')
    parser.add_argument('--config-strongsort', type=str, default='strong_sort/configs/strong_sort.yaml')
    parser.add_argument('--source', type=str, default=r'', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
    parser.add_argument('--conf-thres', type=float, default=0.25, help='confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.5, help='NMS IoU threshold')#55
